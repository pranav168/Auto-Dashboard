    ...
    if auto_mode:
        ...
        if df.isnull().sum().sum() ==0 or continue_nulls:
            ...
            if len(int_list)>0:
                ...
                final_list=int_list+cat_list
                
                # for i in range(1,31):
                #     charts.append(f'chart_{i}')
                #     charts[-1]=st.columns(3)
                   
                # charts=np.array(charts).reshape(-1,3)
                
                # initialize=[]
                # for i in range(1,11):
                #     initialize.append(f'initialize_{i}')
                #     initialize[-1]=charts[i-1]
                
                # print(len(initialize))
                # print(initialize[0][0])

                # for n,i in enumerate(final_list):
                #     plot_df=pd.DataFrame()
                #     plot_df[i]=df[i]
                #     for m,j in enumerate(final_list):
                #         if i != j:
                #             num2=m 
                #             plot_df[j]=df[j]
                #             with charts[m]:
                    ...
                    for j in final_list[n+1:]:
                        if i != j:
                            ...
                            try:
                                ...
                                st.plotly_chart(fig,width=2200,height=900)
                                # st.plotly_chart(fig)
                            except:
                                try:
